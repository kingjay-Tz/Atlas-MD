...
let mergedCommands = ["tiktok", "tiktokdl", "tiktokmp3", "tiktokmp4", "tiktokdoc"];
...
export default {
  ...
  start: async (Atlas, m, { inputCMD, text, prefix, doReact, args }) => {
    if (!text) {
      await doReact("❌");
...
*${prefix}tiktokdoc <link>*`;
        Atlas.sendMessage(m.from, { image: { url: botImage1 }, caption: txtmain }, { quoted: m });
        break;
      }
      ...
      case "tiktokmp3": {
        ...
        try {
          ...
          Atlas.sendMessage(m.from, { audio: { url: data.audio }, mimetype: "audio/mpeg" }, { quoted: m });
        } catch (e) {
          await doReact("❌");
      ...
      case "tiktokmp4": {
        ...
        try {
          const data = await Tiktok(args[0]);
          Atlas.sendMessage(m.from, { video: { url: data.watermark }, caption: `Downloaded by: *${botName}*` }, { quoted: m });
        } catch (e) {
          await doReact("❌");
      ...
      case "tiktokdoc": {
        ...
        try {
          const data = await Tiktok(args[0]);
          Atlas.sendMessage(m.from, { document: { url: data.audio }, mimetype: "audio/mpeg", fileName: `Downloaded by ${botName}.mp3` }, { quoted: m });
        } catch (e) {
          await doReact("❌");
