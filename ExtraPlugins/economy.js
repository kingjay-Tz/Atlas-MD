...
export default {
  ...
  start: async (
    Atlas,
    m,
    { pushName, prefix, inputCMD, doReact, text, args, mentionByTag }
  ) => {
    ...
    switch (inputCMD) {
      ...
      case "deposit": {
        await doReact("💵");
        if (!text) {
          return m.reply(`Please provide an amount to deposit !\n\nExample: *${prefix}deposit 1000*`);
        }
        const user = m.sender;
        const num = parseInt(text);
        const deposit = await eco.deposit(user, cara, num);
        if (deposit.noten)
          return m.reply(`*Your Deposit amount should be less than or equal to your wallet balance!*`);
        await Atlas.sendMessage(
          m.from,
          {
            image: debitCard,
      ...
      case "gamble": {
        ...
        const opp = texts[1];
        const value = texts[0] ? texts[0].toLowerCase() : "";
        ...
        if (!g) return m.reply(`*You don't have sufficient 🪙 Diamond to gamble with*`);
        if (a) return m.reply(`*Sorry ${m.pushName}, you can only gamble with more than 🪙50.*`);
        if (r == opp) {
      ...
      case "lb": {
        await doReact("📊");
        try {
          const h = await eco.lb("cara", 10);
          if (h.length === 0) {
            return Atlas.sendMessage(m.from, { text: "No users found on leaderboard." }, { quoted: m });
          }
          let str = `*\[ ${botName} Leaderboard \]*\n\n`;
          const arr = [];
          for (let i = 0; i < h.length; i++) {
            const username = await userData.findOne({ id: h[i].userID, name: pushname });
            let tname;
          ...
          console.error(err);
          return Atlas.sendMessage(m.from, { text: `An internal error occurred while fetching the leaderboard.` }, { quoted: m });
        }
        break;
      }
      ...
      case "rob": {
        await doReact("💶");
        if (!text) {
          return m.reply(`Please specify the user you want to rob!\n\nExample: *${prefix}rob @user*`);
        }
        ...
        if (k > balance1.wallet) return m.reply(`*☹️ You don't have enough money to pay fine incase you get caught*`);
        if (k > balance2.wallet) return m.reply(`*☹️ Your target doesn't have enough money to rob*`);
        switch (random) {
          case "ran":
      ...
      case "transfer": {
        ...
        if (!text && !m.quoted) {
          return Atlas.sendMessage(m.from, { text: `Please tag any user ${pushName} senpai 🤦‍♂️ !` }, { quoted: m });
        }
        const mentionedUser = m.quoted ? m.quoted.sender : mentionByTag[0];
        const user1 = m.sender;
        const user2 = mentionedUser;
        const word = value[0];
        const d = parseInt(word);
        if (!d) return m.reply("check your text plz u r using the command in a wrong way👀");
        const balance = await eco.balance(user1, cara);
        if (balance.wallet < parseInt(word)) return m.reply("you dont have sufficient money to transfer👎");
        await eco.deduct(user1, cara, value[0]);
        await eco.give(user2, cara, value[0]);
        await Atlas.sendMessage(
          m.from,
          { image: debitCard, caption: `*📠 Transaction successful of ${word} 💷*` },
          { quoted: m }
        );
        break;
      }
      ...
      case "withdraw": {
        ...
        Atlas.sendMessage(
          m.from,
          { image: debitCard, caption: `*🏧 ALERT* _💶 ${withdraw.amount} has been added in your wallet._` },
          { quoted: m }
        );
        break;
      }
